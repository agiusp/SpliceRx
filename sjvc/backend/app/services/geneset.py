...
For a junction matrix the symbols are resolved to GENCODE genes (loci are
needed to find overlapping junctions). For an already gene-level matrix the
symbols are matched straight against the matrix's row labels.
"""
...
class GeneSet:
    ...
    def n_genes(self) -> int:
        return len(self.matched) or len(self.matched_names)


def _label_parts(label: str) -> List[str]:
    """A matrix row label may join several gene symbols with commas, e.g.
    'CFH,CFHR1,CFHR3' — split it so any part can be matched."""
...
def match_matrix_labels(
    ...
    for lab in matrix_labels:
        ordered.append(lab)
        by_full.setdefault(str(lab).strip().lower(), []).append(lab)
        for part in _label_parts(lab):
