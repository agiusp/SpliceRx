...
class Matrix:
    ...
    def restrict_samples(self, keep: Sequence[str]) -> "Matrix":
        """A new Matrix with only the given sample columns, in this matrix's
        existing column order (``keep`` may be a superset or any order)."""
        keep_set = set(keep)
