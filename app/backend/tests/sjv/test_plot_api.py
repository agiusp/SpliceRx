...
def test_sparse_matrix_plots_identically(client, tmp_path, monkeypatch):
    """A dgCMatrix RDS goes through the sparse reader but produces the same plot
    as the equivalent dense matrix."""
    monkeypatch.setenv("SJV_CACHE_DIR", str(tmp_path / "cache"))
    sid = client.post("/api/session").json()["session_id"]
    up = client.post(f"/api/session/{sid}/rds",
                     files={"file": ("m.rds", open(FIXTURES / "mini_sparse.rds", "rb"))})
    assert up.status_code == 200, up.text
    assert up.json()["sparse"] is True
    assert up.json()["n_junctions"] == 10
    client.post(f"/api/session/{sid}/gtf",
                files={"file": ("mini.gtf", open(FIXTURES / "mini.gtf", "rb"))})
    ...
    cats = {a["id"]: a["category"] for a in body["arcs"]}
