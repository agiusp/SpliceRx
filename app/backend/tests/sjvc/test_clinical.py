...
from sjvc.services.clinical import parse_clinical
from sjvc.services.rds import MatrixError
...
def test_rownames_column_used_as_sample_id_when_no_sample_id_column(tmp_path):
    ...
    read_table); this must be usable as the sample_id, not just rejected."""
    p = tmp_path / "t.csv"
    p.write_text("rownames,subtype\ns01,LUAD\ns02,LUSC\n")
    clin = parse_clinical(p, tmp_path, SAMPLES)
    assert clin.n_rows == 2
    assert clin.rows["s01"]["subtype"] == "LUAD"
    assert "rownames" not in {c.name for c in clin.columns}   # consumed as the id, not a feature


def test_real_sample_id_column_wins_over_rownames(tmp_path):
    p = tmp_path / "t.csv"
    p.write_text("rownames,sample_id,subtype\nbogus1,s01,LUAD\nbogus2,s02,LUSC\n")
    clin = parse_clinical(p, tmp_path, SAMPLES)
    assert clin.n_rows == 2
    assert "s01" in clin.rows and "bogus1" not in clin.rows
