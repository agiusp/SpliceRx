...
import { api, ApiError, type FeaturesResponse, type SjdatKind } from "../api";
...
interface Props {
  ...
  activeSjdat: SjdatKind | null;
  /** Junction metadata is loaded — typed/list/pathway lookups against a
   *  junction-level matrix work without a GENCODE reference. */
...
export default function GeneSetPanel({
  ...
  const [tab, setTab] = useState<GeneSetMode>("mad"); // MAD is the only mode that's always usable
  ...
  // N/X only make sense for a raw per-junction matrix — a coverage prefilter
  // over an already-condensed gene/pathway score isn't meaningful, and
  // SJSurv's own MAD ranking already works fine over every row without it.
  ...
  // Meaningless for pathway_matrix, same restriction as the MAD-only tab there.
  ...
  // "top by MAD" mode doesn't execute here at all; it only reports its
  // parameters upward and runs later, from the parent's "Select features".
  useEffect(() => {
    onModeChange(tab);
    setResult(null);
    setFeatures(null);
  ...
  // against the old one, same as switching mode.
  useEffect(() => {
    setResult(null);
    setFeatures(null);
    setErr(null);
  ...
  return (
    <div>
      ...
      {!pathwayMatrixOnly && !geneLevel && !noGencodeNeeded && tab !== "mad" && (
        <div className="muted" style={{ marginBottom: 8, fontSize: 13 }}>
          Select a GENCODE release or upload a GTF on the Data tab first (or load the junction
          metadata file there for a faster, no-GENCODE-needed lookup) to type genes, upload a
          list, or search a pathway against this junction-level matrix.
        </div>
      )}
      ...
      {tab !== "mad" && !pathwayMatrixOnly && (geneLevel || noGencodeNeeded) && (
      ...
      {tab === "typed" && (
        <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
          <textarea
            rows={2}
            disabled={!geneLevel && !noGencodeNeeded}
          ...
          />
          <button
            disabled={busy || !text.trim() || (!geneLevel && !noGencodeNeeded)}
            onClick={() => submit({ mode: "typed", text, prefix })}
          >
            Resolve
          </button>
        </div>
        ...
        <div className="row">
          <label>
            ...
            <span className="file-trigger">
              <span className={`file-btn${!geneLevel && !noGencodeNeeded ? " disabled" : ""}`}>Choose File</span>
              <input
                type="file"
      ...
      {tab === "mad" && (
        <>
          <p className="muted" style={{ marginTop: 0, fontSize: 13 }}>
            {showCoverage ? (
              <>
                A feature is kept when at least <b>N</b> of the group's samples have a non-zero
                entry that is also <b>≥ X</b>; of those, the top <b>n</b> by {rankStat} are used.
              </>
            ) : (
              <>
                The top <b>n</b> {noun} by {rankStat} are used
          ...
          {isRrs && (
            <div className="row" style={{ marginTop: 8 }}>
              ...
              <span className="muted" style={{ maxWidth: 400 }}>
                {junctionCountsLoaded
                  ? "Keeps an RRS junction only when its corresponding row in the loaded junction counts matrix has a max read count above this, across the candidate samples."
      ...
      {tab !== "mad" && features && (
        <div style={{ marginTop: 8, fontSize: 13 }}>
          ...
          × {features.n_samples} samples — ready to select for the chosen group below.
