...
export const api = {
  async createSession(): Promise<string> {
    const d = await j<{ session_id: string }>(await fetch("/api/sjv/session", { method: "POST" }));
    return d.session_id;
  },

  async sessionState(sid: string): Promise<SessionState> {
    return j(await fetch(`/api/sjv/session/${sid}/state`));
  },

  async uploadRds(sid: string, file: File): Promise<RdsUploaded> {
    const fd = new FormData();
    fd.append("file", file);
    return j(await fetch(`/api/sjv/session/${sid}/rds`, { method: "POST", body: fd }));
  },

  async releases(): Promise<Record<string, string[]>> {
    const d = await j<{ releases: Record<string, string[]> }>(await fetch("/api/sjv/gencode/releases"));
    return d.releases;
  },

  async selectGencode(sid: string, species: string, release: string): Promise<{ label: string; warnings: string[] }> {
    return j(
      await fetch(`/api/sjv/session/${sid}/gencode`, {
        method: "POST",
  ...
  async uploadGtf(sid: string, file: File): Promise<{ label: string; warnings: string[] }> {
    const fd = new FormData();
    fd.append("file", file);
    return j(await fetch(`/api/sjv/session/${sid}/gtf`, { method: "POST", body: fd }));
  },

  async suggestGenes(sid: string, q: string): Promise<string[]> {
    if (!q.trim()) return [];
    const d = await j<{ names: string[] }>(
      await fetch(`/api/sjv/session/${sid}/genes/suggest?q=${encodeURIComponent(q)}`),
    );
    return d.names;
  },
